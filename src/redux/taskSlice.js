...
export const createTask = createAsyncThunk(
  ...
  async (taskdata, { rejectWithValue }) => {
    ...
    try {
      ...
      // if (!data.ok) throw new Error("Failed to submit task");
      ...
      toast.error("Task created failure ❌", {
        position: "top-right",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: false,
  ...
  reducers: {
    ...
    getisComplete: (state, action) => {
      ...
      if (action.payload === "All") {
        ...
        state.tasks = state.alltasks.filter(
          (item) => item.isComplete == action.payload
        );
      }
    },
...
export const { getPriority, getisComplete, getLayout, getlayoutmodal } =
  taskSlice.actions;
